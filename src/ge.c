...
char *parservar(char *intext){
	...
	while (intext[a] != '\0'){
		...
		if (intext[a] == '%'){
			a++;
			b = 0;
			memset(xvarname,0, sizeof(xvarname));
			while(intext[a]!=' ' && intext[a]!='\0'){
				xvarname[b++] = intext[a++];
			}
...
void GE_load_fonts(void){
	...
	fonts = ini_get_int("global", "fonts", 1) - 1;
	for (int nf = 0; nf <= fonts; nf++){
		...
		ini_get_color(&font[nf].color,  sfont, "color", "00,FF,00,00");

		// font[nf].color.a=a;
		font[nf].color.a = 0; // Verificar se transparencia funciona;
		if ((font[nf].body = TTF_OpenFont(filenamefont, ini_get_int( sfont, "size", 12))) == NULL){
			ERR("Error opening file:%s", filenamefont);
			exit(1);
		}

		fstyle = 0;
		fstyle = ini_get_int( sfont, "bold", false) * 1;
		fstyle += ini_get_int( sfont, "italic", false) * 2;
		fstyle += ini_get_int( sfont, "underline", false) * 4;
		fstyle += ini_get_int( sfont, "strikethrough", false) * 8;
		TTF_SetFontStyle(font[nf].body, fstyle);
		TTF_SetFontOutline(font[nf].body, ini_get_int( sfont, "outline", 0));

		char hinting[7];
		LETS(hinting, ini_get_str( sfont, "hinting", "normal"));
...
void GE_load_debug_box(void){
    ...
    if(showbox){
		ini_get_color(&sptboxcolor, "global", "sptboxcolor", "ff,ff,ff,ff");
   		ini_get_color(&attackboxcolor, "global", "attackboxcolor", "ff,ff,ff,ff");
	    ini_get_color(&defenseboxcolor,  "global", "defenseboxcolor", "ff,ff,ff,ff");
	}
}
...
void GE_load_sprites(void){
	//===Load sprites==========================================================================================v
	DBG("GE_loadsprites()");
	for (sprites = 0; sprites < ini_get_int( "global", "sprites", 1); sprites++){
		...
		MALETS(GE_sprite[sprites].filestart, ini_get_str( spriteN, "start", ""));
		MALETS(GE_sprite[sprites].fileupdate, ini_get_str( spriteN, "update", ""));
		MALETS(GE_sprite[sprites].fileend, ini_get_str( spriteN, "end", ""));

		char spritetype[10];
		LETS(spritetype, ini_get_str( spriteN, "type", "character"));
		...
		GE_sprite[sprites].flip = ini_get_bool( spriteN, "flip", "false");
		GE_sprite[sprites].tick = ini_get_int( spriteN, "tick", 0);
		GE_sprite[sprites].life = ini_get_int( spriteN, "life", 0);
		GE_sprite[sprites].showbox = ini_get_int( spriteN, "showbox", showbox);
		GE_sprite[sprites].pos.x = ini_get_int( spriteN, "x", 0);
		GE_sprite[sprites].pos.y = ini_get_int( spriteN, "y", 0);
		GE_sprite[sprites].pos.w = ini_get_int( spriteN, "w", 0);
		GE_sprite[sprites].pos.h = ini_get_int( spriteN, "h", 0);
		GE_sprite[sprites].boxattack.x = ini_get_int( spriteN, "attack_x", 0);
		GE_sprite[sprites].boxattack.y = ini_get_int( spriteN, "attack_y", 0);
		GE_sprite[sprites].boxattack.w = ini_get_int( spriteN, "attack_w", 0);
		GE_sprite[sprites].boxattack.h = ini_get_int( spriteN, "attack_h", 0);
		GE_sprite[sprites].boxdefense.x = ini_get_int( spriteN, "defense_x", 0);
		GE_sprite[sprites].boxdefense.y = ini_get_int( spriteN, "defense_y", 0);
		GE_sprite[sprites].boxdefense.w = ini_get_int( spriteN, "defense_w", 0);
		GE_sprite[sprites].boxdefense.h = ini_get_int( spriteN, "defense_h", 0);
		GE_sprite[sprites].animationidx = ini_get_int( spriteN, "defaultanimation", 1) - 1;
		...
		if (GE_sprite[sprites].spritetype == mosaic){
			...
			ini_get_color(&mtcolor, spriteN, "color", "ff,00,00,ff");
     ...
     DBG("GENSPRITE [sprite%d]",sprites+1);
			...
			GE_sprite[sprites].animation[0].textFrame[0] = gensprite(
				spriteN,
				rendscr
			);
			...
			for (int animations = 0;
				 (GE_sprite[sprites].spritetype == character 
				 || GE_sprite[sprites].spritetype == prop)
				 && animations < ini_get_int( spriteN, "animations", 1);
				 animations++)
			{
				char paramx[20];
				char sanitype[20];
				LETSF(paramx, "anima%dtype", animations + 1)
				LETS(sanitype, ini_get_str( spriteN, paramx, "none"));
				...
				GE_sprite[sprites].animation[0].speed = ini_get_int( spriteN, animationSTUFFS, 150);

				LETSF(animationSTUFFS, "anima%dframes", animations + 1);
				GE_sprite[sprites].animation[animations].maxframes = ini_get_int( spriteN, animationSTUFFS, 1);
				...
				for (int frame = 0; frame < GE_sprite[sprites].animation[animations].maxframes; frame++){
					...
					LETS(filename, ini_get_str( spriteN, animaNframeN, "./img/not_found.png"));
...
void GE_create_clone_sprites(){
	//==Load bullets from GE_sprite[SPT_BULLET]============================================================================v
	LETS(filename, ini_get_str( "sprite4", "anima1frame1", "./img/not_found.png"));
...
int main(int argc, char *argv[]){
	...
	width = ini_get_int( "global", "w", 1024);
	height = ini_get_int( "global", "h", 768);

	if (SDL_CreateWindowAndRenderer(width, height, SDL_RENDERER_ACCELERATED |SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS, &windscr, &rendscr)){
    ...
    //--Init ID Window-----v
	char project[200];
	LETS(project, ini_get_str( "global", "name", "GEngine version:" PG_VER));

	SDL_SetWindowTitle(windscr, (const char *)project);

	char fileicon[25];
	LETS(fileicon, ini_get_str( "global", "icon", "./img/GE.png"));
	...
	// GE_create_clone_sprites(); // look above, under load start scripts.
	...
	MALETS(filescript, ini_get_str( "scripts", "start", ""));
	...
	MALETS(filescript, ini_get_str( "scripts", "update", ""));
	if (strlen(filescript)){
		gupdateok = true;
	...
	MALETS(filescriptend, ini_get_str( "scripts", "end", ""));
